...
import { RunProgressClient } from "./_components/RunProgressClient";
...
async function setPaused(formData: FormData) {
  ...
  const { data: evt, error: evtErr } = await supabase
    ...
    .eq("slug", slug)
    .maybeSingle();
  if (evtErr) redirect(`/admin/events/${slug}?error=${encodeURIComponent(evtErr.message)}`);
  if (!evt) redirect(`/admin/events/${slug}?error=${encodeURIComponent("event_not_found")}`);
  ...
  if (!paused) {
    ...
    const { data: runningRows, error: runningErr } = await supabase
      ...
      .limit(1);
    if (runningErr) redirect(`/admin/events/${slug}?error=${encodeURIComponent(runningErr.message)}`);
  ...
  const { error } = await supabase
    ...
    .eq("id", evt.id);
  if (error) redirect(`/admin/events/${slug}?error=${encodeURIComponent(error.message)}`);
  ...
  if (!paused) {
    ...
    if (!isTelegram) {
      ...
      if (rErr || !(runningNow ?? []).length) {
        ...
        redirect(
          `/admin/events/${slug}?error=${encodeURIComponent(
    ...
    <div className="min-h-screen bg-zinc-50 px-6 py-14 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-6">
        <header className="flex items-start justify-between gap-4">
          <div>
            <Link className="text-sm text-zinc-600 underline hover:no-underline dark:text-zinc-400" href="/admin/events">
              ← Ingestion
            </Link>
