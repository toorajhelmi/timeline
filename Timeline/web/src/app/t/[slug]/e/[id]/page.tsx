...
import TimelineEntryMedia from "../../_components/TimelineEntryMedia";
...
export async function generateMetadata({
  ...
  const canonicalUrl = `${siteUrl}/t/${encodeURIComponent(slug)}/e/${encodeURIComponent(id)}`;
...
async function addComment(formData: FormData) {
  ...
  if (!user) redirect(`/login?next=${encodeURIComponent(`/t/${slug}/e/${entryId}`)}`);
...
async function setPinned(formData: FormData) {
  ...
  const { data: timeline, error: tErr } = await supabase
    ...
    .eq("slug", slug)
    .maybeSingle();
  if (tErr || !timeline) redirect(`/t/${slug}/e/${entryId}?error=timeline_not_found`);
  if (timeline.created_by !== user.id) redirect(`/t/${slug}/e/${entryId}?error=not_owner`);

  const service = createSupabaseServiceClient();
  if (!service) redirect(`/t/${slug}/e/${entryId}?error=missing_service_role`);
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-6">
        <header className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <ShareMenu
              path={`/t/${timeline.slug}/e/${entry.id}`}
        ...
        <SharePanel
          fallbackPath={`/t/${timeline.slug}/e/${entry.id}`}
          title={entry.title ?? null}
          body={displayBody}
        />
