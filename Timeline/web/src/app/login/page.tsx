...
async function signInWithPassword(formData: FormData) {
  ...
  if (!password) {
    redirect(`/login?error=missing_password&next=${encodeURIComponent(next)}`);
  }

  const configured = hasPublicSupabaseEnv();
  if (!configured) {
    redirect(`/login?error=missing_supabase_env&next=${encodeURIComponent(next)}`);
  }
      ...
      <main className="mx-auto w-full max-w-xl">
        ...
        >
          ...
          <div className="mt-4 text-center text-sm text-zinc-600 dark:text-zinc-400">
            New here?{" "}
            <Link className="font-medium underline hover:no-underline" href={`/signup?next=${encodeURIComponent(next)}`}>
              Create an account
            </Link>
          </div>
        </form>
