        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          {(["evidence", "claim", "call_to_action"] as EntryType[]).map((t) => (
            <Link
              ...
              href={`/timelines/${slug}?z=${zoom}&type=${t}`}
            >
              {t === "call_to_action"
                ? "action"
                : t === "claim"
...
export default async function TimelineView({
  ...
  if (timeline.canonical_timeline_id) {
    ...
    if (canonical?.slug) redirect(`/timelines/${canonical.slug}`);
  }
          ...
          <div className="flex shrink-0 items-center gap-2">
            {canEdit ? (
              <Link
                ...
                href={`/timelines/${timeline.slug}/settings`}
              >
                Edit timeline
              </Link>
            ) : null}
            <Link
              ...
              href={`/timelines/${timeline.slug}/add`}
              prefetch
            >
              Add entry
            </Link>
