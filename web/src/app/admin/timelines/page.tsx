...
async function setCanonical(formData: FormData) {
  ...
  if (!duplicateSlug) redirect("/admin/timelines?error=missing_duplicate_slug");
  ...
  const { data: dup, error: dupErr } = await supabase
    ...
    .maybeSingle();
  if (dupErr || !dup) redirect("/admin/timelines?error=duplicate_not_found");
  ...
  const { data: canon, error: canonErr } = await supabase
    ...
    .maybeSingle();
  if (canonErr || !canon) redirect("/admin/timelines?error=canonical_not_found");
                ...
                {(timelines ?? []).map((t) => (
                  <tr key={t.id} className="border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <td className="py-3 pr-4 font-mono text-xs">
                      <Link className="underline hover:no-underline" href={`/admin/timelines/${t.slug}`}>
                        {t.slug}
                      </Link>
                    </td>
