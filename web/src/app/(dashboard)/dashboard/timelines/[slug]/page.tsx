...
async function pinEntry(formData: FormData) {
  ...
  if (!timelineId || !entryId || !slug) redirect("/dashboard/timelines");
  ...
  const { data: timeline, error: tErr } = await supabase
    ...
    .eq("id", timelineId)
    .maybeSingle();
  if (tErr || !timeline?.created_by) redirect("/dashboard/timelines?error=timeline_not_found");
  ...
  redirect(`/dashboard/timelines/${slug}`);
}
...
async function setEntryLock(formData: FormData) {
  ...
  if (!slug || !entryId) redirect("/dashboard/timelines");
...
async function setEntryStatus(formData: FormData) {
  ...
  if (!slug || !entryId || !status) redirect("/dashboard/timelines");
...
export default async function AdminTimelineDetailPage({
  ...
  if (timelineError) {
    redirect(`/dashboard/timelines?error=${encodeURIComponent(timelineError.message)}`);
  }
