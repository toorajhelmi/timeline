...
async function requestSignupCode(formData: FormData) {
  ...
  if (!email) redirect(`/auth/signup?error=missing_email&next=${encodeURIComponent(next)}`);

  if (!hasPublicSupabaseEnv()) {
    redirect(`/auth/signup?error=missing_supabase_env&next=${encodeURIComponent(next)}`);
  }
  ...
  redirect(
    `/auth/signup?sent=1&email=${encodeURIComponent(email)}&next=${encodeURIComponent(next)}`,
  );
}
...
async function completeSignup(formData: FormData) {
  ...
  if (!token)
    redirect(
      `/auth/signup?sent=1&email=${encodeURIComponent(email)}&error=missing_code&next=${encodeURIComponent(next)}`,
    );
  if (!password || password.length < 8) {
    redirect(
      `/auth/signup?sent=1&email=${encodeURIComponent(email)}&error=${encodeURIComponent(
        "password_too_short",
    ...
    <div className="min-h-screen bg-zinc-50 px-6 py-14 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50">
      <main className="mx-auto w-full max-w-xl">
        <header className="flex items-center justify-between">
          ...
          <Link className="text-sm font-medium underline hover:no-underline" href="/auth/login">
            Sign in
          </Link>
        </header>
