...
async function updateTheme(formData: FormData) {
  ...
  if (!userData.user) {
    redirect(`/auth/login?next=${encodeURIComponent(`/timelines/${slug}/settings`)}`);
  }
  ...
  if (error) {
    redirect(`/timelines/${slug}/settings?error=${encodeURIComponent(error.message)}`);
  }

  redirect(`/timelines/${slug}/settings?saved=1`);
}
...
async function setVisibility(formData: FormData) {
  ...
  if (!desired) redirect(`/timelines/${slug}/settings?error=${encodeURIComponent("invalid_visibility_intent")}`);
  ...
  if (nextVisibility === "limited") {
    redirect(`/timelines/${slug}/settings?requested_public=1`);
  }
